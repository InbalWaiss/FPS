...
class AlgorithmicEnv(Env):
    ...
    def __init__(self, base=10, chars=False, starting_min_length=2):
        ...
        self.last = 10
        # Cumulative reward earned this episode_to_enemy
    ...
    @property
    def time_limit(self):
        """If an agent takes more than this many timesteps, end the episode_to_enemy
        immediately and return a negative reward."""
        # (Seemingly arbitrary)
        return self.input_width + len(self.target) + 4

    def _check_levelup(self):
        """Called between episodes. Update our running record of episode_to_enemy rewards
        and, if appropriate, 'level up' minimum input length."""
        if self.episode_total_reward is None:
            # This is before the first episode_to_enemy/call to reset(). Nothing to do.
            return
