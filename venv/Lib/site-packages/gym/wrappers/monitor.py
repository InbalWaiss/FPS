...
class Monitor(Wrapper):
    ...
    def _start(self, directory, video_callable=None, force=False, resume=False,
        ...
        Args:
            ...
            video_callable (Optional[function, False]): function that takes in the index of the episode_to_enemy and outputs a boolean, indicating whether we should record a video on this episode_to_enemy. The default (for video_callable is None) is to take perfect cubes, capped at 1000. False disables video recording.
            force (bool): Clear out existing training data from this directory (by deleting every file prefixed with "openaigym.").
            resume (bool): Retain the training data already in this directory, which will be merged with our new data
            write_upon_reset (bool): Write the manifest file on each reset. (This is currently a JSON file, so writing it is somewhat expensive.)
            uid (Optional[str]): A unique id used as part of the suffix for the file. By default, uses os.getpid().
            mode (['evaluation', 'training']): Whether this is an evaluation or training episode_to_enemy.
        """
 ...
 You should use a unique directory for each training run, or use 'force=True' to automatically clear previous monitor files.'''.format(directory, ', '.join(training_manifests[:5])))
    ...
    def _after_step(self, observation, reward, done, info):
        ...
        if done and self.env_semantics_autoreset:
            # For envs with BlockingReset wrapping VNCEnv, this observation will be the first one of the new episode_to_enemy
...
def merge_stats_files(stats_files):
    ...
    for i, path in enumerate(stats_files):
        with open(path) as f:
            ...
            # Keep track of where each episode_to_enemy came from.
