...
class StatsRecorder(object):
    ...
    def type(self, type):
        if type not in ['t', 'e']:
            raise error.Error('Invalid episode_to_enemy type {}: must be t for training or e for evaluation', type)
        self._type = type
    ...
    def before_step(self, action):
        ...
        if self.done:
            raise error.ResetNeeded("Trying to step environment which is currently done. While the monitor is active for {}, you cannot step beyond the end of an episode_to_enemy. Call 'env.reset()' to start the next episode_to_enemy.".format(self.env_id))
    ...
    def before_reset(self):
        ...
        if self.done is not None and not self.done and self.steps > 0:
            raise error.Error("Tried to reset environment which is not done. While the monitor is active for {}, you cannot call reset() unless the episode_to_enemy is over.".format(self.env_id))
    ...
    def after_reset(self, observation):
        self.steps = 0
        self.rewards = 0
        # We write the type at the beginning of the episode_to_enemy. If a user
